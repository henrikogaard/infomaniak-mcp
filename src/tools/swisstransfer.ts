...
export function registerSwissTransferTools(server: McpServer, st: SwissTransferService) {
  server.tool(
    "swisstransfer_send",
    "Send files via Swiss Transfer (encrypted, up to 50GB). Initializes a transfer and uploads files. Returns a download link.",
    {
    ...
    safeHandler(async ({ files, message, recipients, password, expiration_days, download_limit }) => {
  ...
  server.tool(
    "swisstransfer_info",
    "Get information about a Swiss Transfer (status, files, download count)",
    { transfer_id: z.string().describe("Transfer ID") },
    safeHandler(async ({ transfer_id }) => {
